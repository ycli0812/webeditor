...
function Resistor(props) {
    const { x, y, zoom, wireWidth, id, gridX, gridY, selected, active, onMove, onSelect } = props; // x和y是grid坐标
    const gridSize = zoom * 5;

    const [pixelX, setPixelX] = useState(x * gridSize); // grid坐标系中的像素坐标
    const [pixelY, setPixelY] = useState(y * gridSize); // grid坐标系中的像素坐标
    const [initOffset, setInitOffset] = useState({x: 0, y: 0}); // 鼠标按下时指针在元件内部的偏移

    useEffect(() => {
        setPixelX(x * gridSize);
        setPixelY(y * gridSize);
    }, [zoom]);

    function handleMouseDown(ev){
        console.log(id, ': mouse down', x, y);
        ev.stopPropagation();
        onSelect(id, true);
        setInitOffset({x: ev.nativeEvent.offsetX - gridX - pixelX, y: ev.nativeEvent.offsetY - gridY - pixelY});
    }
    ...
    function handleMouseOut(ev) {
        ev.preventDefault();
        if(selected) {
            let gridOffsetX = ev.nativeEvent.offsetX - gridX, gridOffsetY = ev.nativeEvent.offsetY - gridY; // 指针在grid坐标系中的像素坐标
